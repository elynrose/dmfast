        ...
        <div class="col-md-12">
            @can('total_credit_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.total-credits.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $totalCredit->id }}">
                                        ...
                                        <td>
                                            @can('total_credit_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.total-credits.show', $totalCredit->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('total_credit_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.total-credits.edit', $totalCredit->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('total_credit_delete')
                                                <form action="{{ route('frontend.total-credits.destroy', $totalCredit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('total_credit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.total-credits.massDestroy') }}",

        ...
        <div class="col-md-12">
            @can('payment_setting_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.payment-settings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $paymentSetting->id }}">
                                        ...
                                        <td>
                                            @can('payment_setting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.payment-settings.show', $paymentSetting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('payment_setting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.payment-settings.edit', $paymentSetting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('payment_setting_delete')
                                                <form action="{{ route('frontend.payment-settings.destroy', $paymentSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('payment_setting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.payment-settings.massDestroy') }}",

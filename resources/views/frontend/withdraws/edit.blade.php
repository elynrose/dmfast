                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.withdraws.update", [$withdraw->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="methods[]" id="methods" multiple required>
                                @foreach($methods as $id => $method)
                                    <option value="{{ $id }}" {{ (in_array($id, old('methods', [])) || $withdraw->methods->contains($id)) ? 'selected' : '' }}>{{ $method }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $withdraw->amount) }}" step="0.01" required>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="paid" id="paid" value="1" {{ $withdraw->paid || old('paid', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="comment" id="comment">{{ old('comment', $withdraw->comment) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $withdraw->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                                <div class="invalid-feedback">

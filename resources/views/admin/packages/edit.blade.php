    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.packages.update", [$package->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('credits') ? 'is-invalid' : '' }}" type="number" name="credits" id="credits" value="{{ old('credits', $package->credits) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Package::NOTIFY_ME_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('notify_me') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="notify_me_{{ $key }}" name="notify_me" value="{{ $key }}" {{ old('notify_me', $package->notify_me) === (string) $key ? 'checked' : '' }}>

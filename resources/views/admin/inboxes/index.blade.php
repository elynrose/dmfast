                ...
                <tbody>
                    @foreach($inboxes as $key => $inbox)
                        <tr data-entry-id="{{ $inbox->id }}">
                            ...
                            <td>
                                @can('inbox_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.inboxes.show', $inbox->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('inbox_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.inboxes.edit', $inbox->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('inbox_delete')
                                    <form action="{{ route('admin.inboxes.destroy', $inbox->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">

...
@can('credit_setting_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.credit-settings.create') }}">
                        ...
                        <tr data-entry-id="{{ $creditSetting->id }}">
                            ...
                            <td>
                                @can('credit_setting_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.credit-settings.show', $creditSetting->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('credit_setting_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.credit-settings.edit', $creditSetting->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('credit_setting_delete')
                                    <form action="{{ route('admin.credit-settings.destroy', $creditSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('credit_setting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.credit-settings.massDestroy') }}",
